...
import TcmInteractionsPanel from './components/TcmInteractionsPanel';
...
function App() {
  const [view, setView] = useState<View>('tcm');
    ...
    <div className="app-shell">
      <header className="hero-card card">
        <div className="hero-text">
          <span className="badge">临床试运行 · 心血管抗凝</span>
          <h1>药物相互作用网络</h1>
          <p>
            循证、机制驱动的中药 × 西药相互作用知识库; 同时保留西药 DDI 图谱视图.
            所有内容仅供医师参考, 不替代临床判断.
          </p>
        </div>
        {view === 'ddi' && (
      ...
      <nav className="view-tabs" role="tablist">
        <button
          type="button"
          role="tab"
          aria-selected={view === 'tcm'}
          className={`view-tab ${view === 'tcm' ? 'is-active' : ''}`}
          onClick={() => setView('tcm')}
        >
          中药 × 西药互作
        </button>
        <button
          type="button"
          role="tab"
          aria-selected={view === 'ddi'}
          className={`view-tab ${view === 'ddi' ? 'is-active' : ''}`}
          onClick={() => setView('ddi')}
        >
          西药 DDI 图谱
        </button>
